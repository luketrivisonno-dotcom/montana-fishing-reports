...
// IMPORTANT: Use Production iOS key (starts with 'appl_')
// Get this from RevenueCat Dashboard → Settings → Apps → Your iOS App
const API_KEYS = {
  ios: 'appl_mwAVSRmdWKkGegAwZIlDMvvojuR', // Production iOS API key
...
export const initializePurchases = async () => {
  try {
    ...
    // Warn if using placeholder or test key in production
    if (apiKey.includes('YOUR_PRODUCTION_API_KEY') || apiKey.startsWith('test_')) {
      console.warn('[REVENUECAT] ⚠️ Using test/placeholder API key. Subscriptions will not work.');
      console.warn('[REVENUECAT] ⚠️ Get your Production iOS API key from RevenueCat Dashboard → Settings → Apps');
    }
...
export function useRevenueCat() {
  ...
  const loadOfferings = useCallback(async () => {
    ...
    try {
      ...
      const useOffering = offerings.current || offerings.all?.['default'] || offerings.all?.['Defult'];
      
      if (!useOffering) {
        console.error('[REVENUECAT] ❌ No offering found! Check that "default" offering exists in RevenueCat dashboard.');
        console.error('[REVENUECAT] Available offerings:', Object.keys(offerings.all || {}));
      } else {
        // Find monthly and yearly packages from availablePackages array
        const monthlyPkg = useOffering.availablePackages?.find(p => 
          p.packageType === 'MONTHLY' || p.identifier?.includes('monthly')
        );
        const yearlyPkg = useOffering.availablePackages?.find(p => 
        ...
        if (!monthlyPkg && !yearlyPkg) {
          ...
          console.error('[REVENUECAT] Available packages:', useOffering.availablePackages?.map(p => p.identifier) || 'none');
        } else {
          console.log('[REVENUECAT] ✅ Using offering:', useOffering.identifier);
          console.log('[REVENUECAT] Monthly:', monthlyPkg?.product?.identifier || 'NOT FOUND');
          console.log('[REVENUECAT] Yearly:', yearlyPkg?.product?.identifier || 'NOT FOUND');
        }
      }
  ...
  // Try 'current' first, then fall back to 'default' or 'Defult' offering by ID
  const currentOffering = offerings?.current || offerings?.all?.['default'] || offerings?.all?.['Defult'];
  
  // Find monthly and yearly packages from availablePackages array
  const monthlyPackage = currentOffering?.availablePackages?.find(p => 
    p.packageType === 'MONTHLY' || p.identifier?.includes('monthly')
  );
  const yearlyPackage = currentOffering?.availablePackages?.find(p => 
    p.packageType === 'ANNUAL' || p.packageType === 'YEARLY' || 
    p.identifier?.includes('yearly') || p.identifier?.includes('annual')
  );
