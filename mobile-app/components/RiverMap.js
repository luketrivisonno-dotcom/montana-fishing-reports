...
import MapLongPressModal from './MapLongPressModal';
import RiverSelectorModal from './RiverSelectorModal';
...
export default function RiverMap({ isPremium, onLogCatch, onMapPress, selectionMarker }) {
  ...
  const handleMapLongPress = (e) => {
    // Don't show long press modal if we're in selection mode
    if (onMapPress) return;
    
    // Check if premium - personal pins are premium only
    if (!isPremium) {
      Alert.alert(
        '🔒 Premium Feature',
        'Add personal pins to mark your secret fishing spots, campsites, and more. Upgrade to Premium to unlock!',
        [
          ...
          { 
            ...
            style: 'default',
            onPress: () => {
              // Navigate to paywall - use global function
  ...
  const handleLogCatch = () => {
    setShowLongPressModal(false);
    setShowRiverSelector(true);
  };

  const handleSelectRiverForCatch = (river) => {
    setShowRiverSelector(false);
    // Pass the coordinate and river to parent for logging
    if (onLogCatch) {
      onLogCatch({
        river,
  ...
  return (
    <View style={styles.container}>
      ...
      <RiverSelectorModal
        ...
        onSelectRiver={handleSelectRiverForCatch}
      />
    </View>
  );
}
