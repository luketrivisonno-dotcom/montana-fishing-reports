...
const images = {
  ...
  ClarkFork: require('./ClarkFork.jpeg'),
  Gallatin: require('./GallatinNew.jpeg'),
  Jefferson: require('./Jefferson.jpeg'),
  LowerMadison: require('./LowerMadison2.jpeg'),
  LowerMadisonAlt: require('./LowerMadison.jpeg'),
  Missouri: require('./Missouri.jpeg'),
  RockCreek: require('./RockCreek.jpeg'),
  Ruby: require('./Ruby.jpeg'),
  SpringCreeks: require('./SpringCreeks.jpeg'),
  Stillwater: require('./stillwater.jpeg'),
  Swan: require('./Swan.jpeg'),
  UpperMadison: require('./UpperMadison.jpg'),
  UpperMadison2: require('./UpperMadison2.jpeg'),
  Yellowstone: require('./yellowstone.jpg'),
  Flathead: require('./Flathead.jpg'),
  Boulder: require('./Boulder.jpeg'),
  
  // YNP Rivers - new photos
  Firehole: require('./Firehole.jpeg'),
  SodaButte: require('./SodaButte.jpeg'),
...
const RIVER_IMAGE_MAP = {
  ...
  'Madison River': images.UpperMadison,
  ...
  // Missing rivers - using fallbacks
  'North Fork Flathead River': images.Flathead,
  'Middle Fork Flathead River': images.Flathead,
  'South Fork Flathead River': images.Flathead,
  'Smith River': images.Missouri,
  'Dearborn River': images.Jefferson,
...
export function getRiverImage(riverName) {
  console.log(`[RIVER IMAGE] Looking for: "${riverName}"`);
  
  if (RIVER_IMAGE_MAP[riverName]) {
    console.log(`[RIVER IMAGE] Found mapped image for: ${riverName}`);
    return RIVER_IMAGE_MAP[riverName];
  }
  
  // Try case-insensitive match
  const lowerRiverName = riverName?.toLowerCase();
  for (const [key, value] of Object.entries(RIVER_IMAGE_MAP)) {
    if (key.toLowerCase() === lowerRiverName) {
      console.log(`[RIVER IMAGE] Found case-insensitive match: ${key}`);
      return value;
    }
  }
  
  console.warn(`[RIVER IMAGE] No image found for: "${riverName}" - using fallback`);
