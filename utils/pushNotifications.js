 ...
 * Used for scheduled smart notifications
 */
async function sendBulkNotifications(tokens, title, body, data = {}) {
    ...
    try {
      ...
      if (response.data?.data) {
        for (let i = 0; i < response.data.data.length; i++) {
          ...
          if (item.status === 'ok') {
            successCount++;
            results.push({ token, success: true, id: item.id });
          } else {
            failureCount++;
            results.push({ token, success: false, error: item.message });
  ...
  console.log(`📱 Bulk notifications: ${successCount} sent, ${failureCount} failed (${uniqueTokens.length} total)`);
...
/**
 * Send smart notification based on type
 */
async function sendSmartNotification(tokens, type, data) {
  let title, body;
  
  switch (type) {
    case 'hatchcast_score':
      title = `🎣 ${data.river} is HOT!`;
      body = `HatchCast score: ${data.score}/100 - Excellent fishing conditions!`;
      break;
    case 'flow_spike':
      title = `🌊 Flow Spike on ${data.river}!`;
      body = `Jumped ${data.change} CFS - Fishing will be tough. Try edges & slower water.`;
      break;
    case 'flow_drop':
      title = `⬇️ Flow Dropping on ${data.river}`;
      body = `Down ${Math.abs(data.change)} CFS - Fish are moving to deeper runs`;
      break;
    case 'flow_danger':
      title = `⚠️ High Water Warning: ${data.river}`;
      body = `${data.currentFlow} CFS is dangerous. Safety first - consider postponing.`;
      break;
    case 'weather_wind':
      title = `🌤️ ${data.river} - Perfect Wind Conditions`;
      body = `Wind at ${data.windSpeed}mph - Ideal for delicate dry fly presentations`;
      break;
    case 'weather_temp':
      title = `🌡️ Prime Water Temps on ${data.river}`;
      body = `Water temp at ${data.temp}°F - Trout will be actively feeding!`;
      break;
    case 'solunar_major':
      title = `🌙 Major Feeding Period - ${data.river}`;
      body = `Peak activity predicted in ${data.minutesUntil} mins!`;
      break;
    default:
      title = '🎣 Montana Fishing Reports';
      body = 'New fishing update available';
  }
  
  return sendBulkNotifications(tokens, title, body, { type, ...data });
}
